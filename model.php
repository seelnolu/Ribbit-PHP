...
class Model
{
	...
	{
		$this->db = new mysqli('localhost', 'dev', 'soR7thai', 'Ribbit');
	}
	...
	public function getUserInfo($user) {
		$query = "
			SELECT ribbit_count, IF(ribbit IS NULL, 'You have no Ribbits', ribbit) as ribbit, followers, following
			FROM (
				SELECT COUNT(*) AS ribbit_count FROM Ribbits WHERE user_id = " . $user->id . "
				) AS RC
			LEFT JOIN (
				SELECT user_id, ribbit FROM Ribbits WHERE user_id = " . $user->id . " ORDER BY created_at DESC LIMIT 1
				) AS R
			ON R.user_id = " . $user->id . "
			JOIN (
				SELECT COUNT(*) AS followers FROM Follows WHERE followee_id = " . $user->id . "
				) AS FE
			JOIN (
				SELECT COUNT(*) AS following FROM Follows WHERE user_id = " . $user->id . "
				) AS FR;";
		$res = $this->db->query($query);
		return $res->fetch_object();
	}
	...
	{
		$query = "
			SELECT name, username, gravatar_hash, ribbit, Ribbits.created_at
			FROM Ribbits
			JOIN (
				SELECT Users.*
				FROM Users
				LEFT JOIN (
					SELECT followee_id FROM Follows WHERE user_id = " . $user->id . "
					) AS Follows ON followee_id = id
				WHERE followee_id = id OR id = " . $user->id . "
				) AS Users ON user_id = Users.id
			ORDER BY Ribbits.created_at DESC LIMIT 10;";
		$res = $this->db->query($query);
		$fribbits = array();
	...
	public function getPublicRibbits($q)
	{
		if($q === false) {
			$query = "
				...
				ORDER BY Ribbits.created_at DESC
				LIMIT 10;";
		} else {
			$query = "
				SELECT name, username, gravatar_hash, ribbit, Ribbits.created_at
	...
	public function getPublicProfiles($user, $q)
	{
		if($q === false) {
			...
			$query = "
				SELECT id, name, username, gravatar_hash
				FROM Users
				WHERE id != " . $user->id . " AND (name LIKE \"%" . $q . "%\"
					OR username LIKE \"%" . $q . "%\")
